...
class Trainer():
    ...
    def _optimizer_step(self, imgs, rendered):
        ...
        if self.use_ema:
            # Keep EMA state on CPU to avoid device mismatch when model is on GPU.
            for k, v in self.model.state_dict().items():
                # v may be on GPU; move it to CPU before updating EMA state which we store on CPU
                self.ema_state[k] = 0.999 * self.ema_state[k] + 0.001 * v.detach().cpu()
