...
class SceneRenderDataset(Dataset):
    ...
    def __init__(self, path_to_data='chairs-train', img_transform=None,
        ...
        # parameters and views. Ignore files in the dataset root and only
        # consider directories as scene folders.
        all_paths = glob.glob(path_to_data + '/*')
        self.scene_paths = [p for p in all_paths if os.path.isdir(p)]
        self.scene_paths.sort()  # Ensure consistent ordering of scenes
        if len(self.scene_paths) == 0:
            raise RuntimeError(f"No scene directories found in {path_to_data}; check your dataset layout.")
        self.num_scenes = len(self.scene_paths)
        # Extract number of rendered images per object (which we assume is constant)
        # Find first scene directory that contains pngs
        first_scene_with_pngs = None
        for p in self.scene_paths:
            pngs = glob.glob(p + '/*.png')
            if len(pngs) > 0:
                first_scene_with_pngs = p
                break
        if first_scene_with_pngs is None:
            raise RuntimeError(f"No .png images found under any scene directory in {path_to_data}")
        self.num_imgs_per_scene = len(glob.glob(first_scene_with_pngs + '/*.png'))
        ...
        for scene_path in self.scene_paths:
            ...
            for img_path in img_paths:
                ...
                # Attempt to extract a 5-digit index as in original datasets,
                # but fall back to the full filename stem if necessary. This
                # keeps backward compatibility with datasets that name files
                # e.g. '00001.png' and also supports arbitrary names like
                # 'bowl1.png'.
                ...
                if img_idx not in render_params:
                    raise KeyError(f"Render params for image index '{img_idx}' not found in {scene_path}/render_params.json")
